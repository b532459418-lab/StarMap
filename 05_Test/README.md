...
- Mark as visited (标记为去过) in `dev:personal`: converting a want-to-go city in a new country (for example Nuuk, GL) lands on Map with that city selected after the reload; the new country appears in the country list at its date position, the city has a solid travel marker, the want-to-go entry is gone, and Journey shows the visit. Converting a city in a visited country with **Keep on the Want to Go list** adds it to the existing country (no second copy of the country) and leaves the entry, so the map shows the travel marker with a heart badge. A want-to-go city already in the travel records (for example Reykjavik, IS) shows a disabled button with the already-in-your-travel-records reason in Collection and on the detail card; a direct request to the endpoint is still refused and changes none of `travel-map.local.json`, `editor-state.local.json`, or `want-to-go.local.json`. With the Travel layer turned off, a conversion turns it back on and the new city's marker is visible after the reload. If a write after the travel record fails, the dialog shows the 足迹已创建，但… error and replaces the submit button with Refresh page (刷新页面). Whole-country and coordinate-less entries show a disabled button with the reason. A planned record with coordinates becomes a visited city and leaves the Want to Go layer and Collection; a planned record without coordinates has a disabled button. The dates are never prefilled. `dev:public` and `?data=sample` show no Mark as visited button.
