...
顶部导航的「Collection」页列出想去图层里的全部条目，包括已隐藏的、没有坐标的，以及在地图上并入足迹城市的地点。可以按名称、国家代码或备注搜索，按「显示中 / 已隐藏」筛选，按最近加入、名称或国家排序。「在地图上查看」会切回地图、把镜头飞到该地点并打开详情卡；想去图层被关掉时会自动重新打开。在个人配置中，每张卡片还可以编辑备注、隐藏、恢复或彻底删除；来自旅行记录的 planned 条目与公开样例在这里只读，只有 planned 条目可以「标记为去过」。

去过之后，在个人配置中可以用「标记为去过」把它变成足迹：Collection 卡片的「在地图上查看」旁、地图详情卡的「隐藏」旁都有这个按钮。到访日期由你自己填写（结束日期与行程标题可选），这座城市随即进入足迹，归入这个国家代码在足迹中已有的国家；想去条目默认从列表移除，勾选「保留在想去列表（还想再去）」则保留，去过的城市会显示心形徽标。旅行计划（planned）同样可以标记为去过：这条记录本身改为已去过，日期以你填写的为准，不会新增记录。整个国家与无坐标的条目需要先具体到有坐标的城市，按钮会禁用并说明原因。已经在足迹里的城市，按钮会禁用并说明原因（本地编辑器同样会拒绝这类请求，不写任何文件），这时请改用隐藏或彻底删除。想去备注不会写进旅行记录。转换成功后页面刷新并落在新城市上；足迹图层被关掉时会自动重新打开。

想去条目保存在 `<private-root>/data/want-to-go.local.json`，与其他私人数据一样位于源码仓库之外。个人配置中还没有这个文件时，想去图层为空，不会改用样例。旅行记录里 `status: planned` 的条目也会以只读方式出现在想去图层中；除「标记为去过」外，要修改请直接改旅行记录。

公开构建与 `dev:public` 显示中性的三条样例（努克、特罗姆瑟、阿克雷里；阿克雷里与示例旅程重合，用来演示心形徽标），不包含任何添加、隐藏、删除或「标记为去过」入口。强制样例模式（`VITE_TRAVEL_ATLAS_DATA_MODE=sample`）同样只显示这份样例，没有想去的写入入口。
