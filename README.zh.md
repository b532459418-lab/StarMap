...
顶部导航的「Collection」页列出想去图层里的全部条目，包括已隐藏的、没有坐标的，以及在地图上并入足迹城市的地点。可以按名称、国家代码或备注搜索，按「显示中 / 已隐藏」筛选，按最近加入、名称或国家排序。「在地图上查看」会切回地图、把镜头飞到该地点并打开详情卡；想去图层被关掉时会自动重新打开。在个人配置中，每张卡片还可以编辑备注、隐藏、恢复或彻底删除；来自旅行记录的 planned 条目与公开样例在这里只读。
