...
去过之后，在个人配置中可以用「标记为去过」把它变成足迹：Collection 卡片的「在地图上查看」旁、地图详情卡的「隐藏」旁都有这个按钮。到访日期由你自己填写（结束日期与行程标题可选），这座城市随即进入足迹，归入这个国家代码在足迹中已有的国家；想去条目默认从列表移除，勾选「保留在想去列表（还想再去）」则保留，去过的城市会显示心形徽标。旅行计划（planned）同样可以标记为去过：这条记录本身改为已去过，日期以你填写的为准，不会新增记录。整个国家与无坐标的条目需要先具体到有坐标的城市，按钮会禁用并说明原因。已经在足迹里的城市会被拒绝，不写任何文件，这时请改用隐藏或彻底删除。想去备注不会写进旅行记录。
